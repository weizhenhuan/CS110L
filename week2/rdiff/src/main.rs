    ...
    for i in 0..len1 {
        for j in 0..len2 {
            if seq1[i] == seq2[j] {
                ...
                grid.set(i+1, j+1,
                         max(grid.get(i+1, j).unwrap(),
                             grid.get(i, j+1).unwrap())).unwrap()
            }
        }
    }
    grid
...
fn main() {
    ...
    let file1_vec = read_file_lines(filename1).expect("read file1 error!");
    let file2_vec = read_file_lines(filename2).expect("read file2 error!");
    let grid = lcs(&file1_vec, &file2_vec);

    print_diff(&grid, &file1_vec, &file2_vec, file1_vec.len(), file2_vec.len());
}
